...
import { graphql, GraphQLResolveInfo, GraphQLSchema, specifiedRules } from 'graphql';
import { query, user } from './query/query.js';
...
const plugin: FastifyPluginAsyncTypebox = async (fastify) => {
  ...
  fastify.route({
    ...
    async handler(req) {
      const validationRules = [depthLimit(5), ...specifiedRules];
      ...
      if (validationErrors.length > 0) {
        ...
        return graphql({
          ...
          contextValue: {
              ...
              async (parentIds: readonly string[]) => {
                ...
                const usersByParentId = parentIds.map((parentId) => {
                  return users.filter((user) => {
                    ...
                    if (subscribedToUser) {
                      return subscribedToUser.some(
                        (subscribedTo) => subscribedTo.subscriberId === parentId,
                      );
                    }
                    return false;
                  });
            ...
            subscribedToUserLoader: new DataLoader(
              async (parentIds: readonly string[]) => {
                const users = await prisma.user.findMany({
                  where: {
                    userSubscribedTo: {
                ...
                const usersByParentId = parentIds.map((parentId) => {
                  return users.filter((user) => {
                    ...
                    if (userSubscribedTo) {
                      return userSubscribedTo.some(
                        (subscribedTo) => subscribedTo.authorId === parentId,
                      );
                    }
                    return false;
                  });
